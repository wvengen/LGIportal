...
class LGIUser {
	...
	function set_certkey($cert, $key) {
		...
		if (count($cn)==2) {
			...
			$certuser = $cn[0];
			$certproject = $cn[1];
			$certgroups = explode(',', $cn[2]);
		}
		// update or insert key/certificate
		$query = "%t(usercerts) SET `cert`='%%', `key`='%%', `username`='%%', `fixedgroups`='%%'";
		lgi_mysql_query("UPDATE $query WHERE `user`='%%'",  $cert, $key, $certuser, !is_null($certgroups), $this->userid);
		if (mysql_affected_rows()>0) {
			// TODO if certgroups set, remove them from usergroups (or they will become duplicated)
		} else {
			// new row: add it instead
			lgi_mysql_query("INSERT INTO $query, `user`='%%'",  $cert, $key, $certuser, !is_null($certgroups), $this->userid);
		}
		// create groups
		...
		if ($certgroups!==null) foreach ($certgroups as $g)
